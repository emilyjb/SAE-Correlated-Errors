...
setwd("C:/Users/Emily/Documents/SAE-Correlated-Errors-master-3-22")
library("sae")
library("saeME")
...
for( i in 1:4){ 
	...
	####### degrees of freedom 5 from original submission
      ####### randomgen <- rt(n/2, df = 5)/sqrt(5/3)
	####### degrees of freedom 3 for revision
	randomgen <- rt(n/2, df = 3)/sqrt(3/1)
	####### randomgen <-  (rchisq(n/2, df = 3)-3)/sqrt(6)
...
##### Old save.image from t(5) sims: save.image(paste("abrhosimsUnequalT/Config", outeriter,".Rdata") )
##### Revision save.image from t(3) sims: save.image(paste("abrhosimsUnequalT3/Config", outeriter,".Rdata") )
save.image(paste("abrhosimsUnequalT3/Config", outeriter,".Rdata") )
